  ...
  passport.authenticate("local", (err, user, info) => {
    ...
    return req.login(user, async err => {
      ...
      try {
        ...
        const isChecked = await db.Daily.findOne({
          where: {
            userId: fullUser.id,
            checkDate: moment(new Date().toISOString()).format("YY-MM-DD")
          }
        });
        ...
        if (!isChecked) {
          const today = String(moment(new Date().toISOString()).format("YYYY-MM-DD"))
          await fullUser.addChecking('20');
        }
        console.log("???????????????????????????????????????????????");
        console.log("???????????????????????????????????????????????");
        console.log("???????????????????????????????????????????????");
