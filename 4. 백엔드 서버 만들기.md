...
### 4) front/store/posts 에  uploadImage 작성
...
### 5) back/routes/post.js에 uploadImages를 처리하는 router 작성
...
- 3) 폼데이터나 폼 태그를 통해 업로드한 이미지를 올리면, req.file 로 정보가 들어온다.
...
- 1) back/routes/ 이미지 업로드 router 작성하기
  - 요청의 files에 들어있는 값을 확인한다.

```
[ { fieldname: 'image',
    originalname: 'workbench.PNG',
    encoding: '7bit',
    mimetype: 'image/png',
    destination: 'uploads',
    filename: 'workbench1578644975137.PNG',
    path: 'uploads\\workbench1578644975137.PNG',
    size: 85362 } ]
```

- 2) multer.diskStorage에서 확인을 하면 아래와 같이 변경된다.
...
- 3) req.files에서 filename 만 응답으로 프론트에 보내준다.
- 4) upload에 array를 이미지를 하나가 아닌 여러 개를 받도록 하면, uploads파일에 이미지가 올라간다.
  - front/components/PostForm.vue에서 Formdata의 키로 image로 했으니 array에 image를 담는다.
- 5) 
