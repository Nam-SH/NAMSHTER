...
export const actions = {
  ...
  unfollow({ commit }, payload) {
    ...
    .then((res) => {
      commit('unfollow', { userId: payload.userId })
    })
    .catch((err) => {
  ...
  // 팔로잉, 팔로워 (초기 화면 수정 후)
  loadFollowers: throttle(async function ({ commit, state }, payload) {
    try {
  ...
  loadFollowings: throttle( async function ({ commit, state }, payload) {
    try {
      if (payload && payload.reset) {
        const res = await this.$axios.get(`/user/${state.me.id}/followings?limit=3`, {
          withCredentials: true
        })
        commit('loadFollowings', { data: res.data, reset: true, });
        return;
      }
      if (state.hasMoreFollowing) {
        ...
        commit('loadFollowings', { data: res.data, reset: false, });
        return;
      }
    }
    catch (err) {
