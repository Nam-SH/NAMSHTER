...
- 프론트 배포 고정IP 주소: http://3.20.125.129/ 을 접속하면, 프론트의 화면이 정상적으로 보여진다.
...
- ubuntu에서 서비를 실행 한 뒤, 서버를 끄게 되면, 실제 서버도 종료가 된다.
